...
isOpera = "false";
if(navigator.mimeTypes.length >= 20)
{
	for (var i=20; i<=navigator.mimeTypes.length-1; i++)
	{		
		if((navigator.mimeTypes[i].type.search(/opera.+/)!= -1) && isOpera == "false")
		{
			isOpera = "true";
		}
	}
...
{
	//if(confirm('1Das Angebot setzt das SVG-Plugin 3 von Adobe voraus. Moechten Sie das Plugin jetzt installieren?'))
	//location=getSVGInstallPage();
	if(is_nav)
	{
		location.replace("./help/MozillaHinweise.htm");	
	}
...
if(window.svgInstalled)
	...
	if(navigator.appName=='Netscape') {
		document.writeln('<p>Das Angebot setzt das SVG-Plugin 3 von Adobe voraus. Ihr Browser unterstuetzt die automatische Installation nicht.');
		document.writeln('<a href="./help/MozillaHinweise.htm">Klicken Sie hier</a> fuer weitere Informationen.</p>');
		}
	else
...
{ 	
	...
	location.replace("./help/LinuxHinweise.htm");		}
}
